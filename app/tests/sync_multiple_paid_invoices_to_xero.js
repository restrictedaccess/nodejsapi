                                ...
                                payments: {
                                    savePayments: function (payments){
                                        ...
                                        function createToJson(i){
                                            var current_invoice = all_xero_payments[i];
                                            payments[i].toJSON = function(){
                ...
                Q.allSettled(all_given_creation_promises).then(function(results){
                    ...
                    xero_bull.processBatchPayments({},
                        function(err, response){
                            ...
                            xeroPaymentsObj.getAllData(true).then(function(batch_data){
                                try{
                                    ...
                                    expect(synced_invoice_1.Account.AccountID).to.be.equal("E0A58E94-6524-42D9-B8C8-BCA5DB92790C");
                                    expect(synced_invoice_1.Invoice.InvoiceNumber).to.be.equal("14077-00000006");
                                    expect(synced_invoice_1.Invoice.InvoiceID).to.be.equal("dcf0dfd0-b023-4acb-bb76-1c1f3cba3ae1");
                                    expect(synced_invoice_1.Amount).to.be.equal(1254.33);
                                    expect(synced_invoice_1.Date).to.be.equal("2017-06-21");



                                    expect(synced_invoice_2.Account.AccountID).to.be.equal("E0A58E94-6524-42D9-B8C8-BCA5DB92790C");
                                    expect(synced_invoice_2.Invoice.InvoiceNumber).to.be.equal("14076-00000006");
                                    expect(synced_invoice_2.Invoice.InvoiceID).to.be.equal("dcf0dfd0-b023-4acb-bb76-1c1f3cba3ae0");
                                    expect(synced_invoice_2.Amount).to.be.equal(1254.33);
                                    expect(synced_invoice_2.Date).to.be.equal("2017-06-21");
