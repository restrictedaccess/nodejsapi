...
describe("System syncs invoices to xero.com as Invoice", function(){
    ...
    describe("An admin created an invoice (manually-generated) [Happy Path] When The invoice is saved by the admin", function(){
                        ...
                        function(err, response){
                            ...
                            xeroInvoiceObj.getOneData(invoice_details.order_id, true).then(function(synced_invoice){
                                try{
                                    ...
                                    expect(synced_invoice.AmountDue).to.be.equal(1246.08);
                                    ...
                                    expect(synced_invoice.SubTotal).to.be.equal(1132.8);
                                    expect(synced_invoice.TotalTax).to.be.equal(113.28);
                                    expect(synced_invoice.Total).to.be.equal(1246.08);
                                    ...
                                    expect(first_item.Description).to.be.equal("Andrea Jaeger Sierra [Phone Support Professional (Office Based)] \n\nDate Coverage: [17 Jun 01 to 17 Jun 30]");
                                    expect(first_item.UnitAmount).to.be.equal(7.48);
                                    expect(first_item.Quantity).to.be.equal(160);
                                    expect(first_item.TaxType).to.be.equal("OUTPUT");
                                    expect(first_item.LineAmount).to.be.equal(1196.8);
                                    expect(first_item.TaxAmount).to.be.equal(119.68);
                                    expect(first_item.AccountCode).to.be.equal("RRH");

                                    var second_item = synced_invoice.LineItems[1];
                                    expect(second_item.Description).to.be.equal("Currency Adjustment (Contract Rate 1 AUD = 35.00 PESO vs. Current Rate 1 AUD = 37.00 PESO, Currency Difference of -2.00 PESO for your staff Andrea Jaeger Sierra [Phone Support Professional (Office Based)]) \n\nDate Coverage: [17 Jun 01 to 17 Jun 30]")
                                    expect(second_item.UnitAmount).to.be.equal(-0.4);
                                    expect(second_item.Quantity).to.be.equal(160);
                                    expect(second_item.TaxType).to.be.equal("");
                                    expect(second_item.LineAmount).to.be.equal(-64);
