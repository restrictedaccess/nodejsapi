...
describe("System batch syncs clients to xero.com as contact every 30 seconds", function(){
    ...
    //Scenario 1
    describe("4 clients are updated to the system [Happy Path] When The 30 second syncer is triggered via cronjob", function(){
        it("Xero client will save all clients as Contacts in a batch with a single call", function(done){
            this.timeout(30000);
            ...
            Q.allSettled(all_given_saving_promises).then(function(results){
                ...
                xero_bull.processBatchClients({},
                    function(error, response){
                        ...
                        xeroContactObj.getAllData(true).then(function(synced_clients){

                            xeroContactsToSyncObj.getAllData(true).then(function(batch_data){


                                try{
                                    expect(batch_data).to.have.lengthOf(0);

                                    var synced_client_1 = synced_clients[0];
                                    var synced_client_2 = synced_clients[1];
                                    var synced_client_3 = synced_clients[2];
                                    var synced_client_4 = synced_clients[3];


                                    expect(synced_client_1.ContactID).to.be.equal("33be2ffa-dc57-46ad-a688-4b66ff46c20c");
                                    expect(synced_client_1.ContactNumber).to.be.equal("14648");
                                    expect(synced_client_1.AccountNumber).to.be.equal("SW-14648");
                                    expect(synced_client_1.ContactStatus).to.be.equal("ACTIVE");
                                    expect(synced_client_1.Name).to.be.equal("Stephen Wallace");
                                    expect(synced_client_1.FirstName).to.be.equal("Stephen");
                                    expect(synced_client_1.LastName).to.be.equal("Wallace");
                                    expect(synced_client_1.EmailAddress).to.be.equal("davidimrie02@mcgrath.com.au");
                                    expect(synced_client_1.DefaultCurrency).to.be.equal("AUD");


                                    expect(synced_client_2.ContactID).to.be.equal("33be2ffa-dc57-46ad-a688-4b66ff46c20b");
                                    expect(synced_client_2.ContactNumber).to.be.equal("14647");
                                    expect(synced_client_2.AccountNumber).to.be.equal("MC-14647");
                                    expect(synced_client_2.ContactStatus).to.be.equal("ACTIVE");
                                    expect(synced_client_2.Name).to.be.equal("Marie Curry");
                                    expect(synced_client_2.FirstName).to.be.equal("Marie");
                                    expect(synced_client_2.LastName).to.be.equal("Curry");
                                    expect(synced_client_2.EmailAddress).to.be.equal("davidimrie02@mcgrath.com.au");
                                    expect(synced_client_2.DefaultCurrency).to.be.equal("AUD");


                                    expect(synced_client_3.ContactID).to.be.equal("33be2ffa-dc57-46ad-a688-4b66ff46c20a");
                                    expect(synced_client_3.ContactNumber).to.be.equal("14646");
                                    expect(synced_client_3.AccountNumber).to.be.equal("HT-14646");
                                    expect(synced_client_3.ContactStatus).to.be.equal("ACTIVE");
                                    expect(synced_client_3.Name).to.be.equal("Huston Texas");
                                    expect(synced_client_3.FirstName).to.be.equal("Huston");
                                    expect(synced_client_3.LastName).to.be.equal("Texas");
                                    expect(synced_client_3.EmailAddress).to.be.equal("davidimrie02@mcgrath.com.au");
                                    expect(synced_client_3.DefaultCurrency).to.be.equal("AUD");


                                    expect(synced_client_4.ContactID).to.be.equal("33be2ffa-dc57-46ad-a688-4b66ff46c20f");
                                    expect(synced_client_4.ContactNumber).to.be.equal("14645");
                                    expect(synced_client_4.AccountNumber).to.be.equal("DI-14645");
                                    expect(synced_client_4.ContactStatus).to.be.equal("ACTIVE");
                                    expect(synced_client_4.Name).to.be.equal("David Imrie");
                                    expect(synced_client_4.FirstName).to.be.equal("David");
                                    expect(synced_client_4.LastName).to.be.equal("Imrie");
                                    expect(synced_client_4.EmailAddress).to.be.equal("davidimrie02@mcgrath.com.au");
                                    expect(synced_client_4.DefaultCurrency).to.be.equal("AUD");
