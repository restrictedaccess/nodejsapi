    ...
    describe("Invoice created with subcontractors change of rate", function(){
        ...
        it("should be saving to our system", function(done){
            ...
            db.once("open", function(){
                ...
                all_given_creation_promises.push(helper.generate_timesheet(timesheet_may, "locked", 207, 8.00));
                    ...
                    invoice_auto_creation_process.invoiceCreationQueue({data:{client_id:6071}}, function(err, result){
                        ...
                        InvoiceModel.findOne({}).exec(function(err, createdInvoice){
                            try{
                                ...
                                expect(createdInvoice.sub_total).to.equal(1889.47);
                                expect(createdInvoice.gst_amount).to.equal(188.95);
                                expect(createdInvoice.total_amount).to.equal(2078.42);
                                ...
                                expect(second_item.qty).to.equal(23);
                                expect(second_item.unit_price).to.equal(9.37);
                                expect(second_item.amount).to.equal(215.51);
                                ...
                                expect(third_item.description).to.equal("Currency Adjustment (Contract Rate 1 AUD = 38 PESO VS Current Rate 1 AUD = 37.5 PESO, Currency Difference of 0.5  PESO for your staff Efigenio IV Rodriguez)(Actual Working Hours of 207 from May 1, 2017 to May 31, 2017)(Hourly Rate 9.37)/Current Rate 37.5");
                                ...
                                expect(third_item.qty).to.equal(207);
                                expect(third_item.unit_price).to.equal(0.12);
                                expect(third_item.amount).to.equal(24.84);
