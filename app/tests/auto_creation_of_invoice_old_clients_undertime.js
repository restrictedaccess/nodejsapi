...
describe("Auto Creation of Invoice for Old Client", function() {
    ...
    //Scenario 2
    describe("Created invoice inserted to client_docs", function(){
        ...
        it("should be saving to our system", function(done) {
            ...
            db.once("open",function(){
                ...
                var timesheet_june = {
                    ...
                    date_generated: moment_tz("2017-06-cl:53:26Z").toDate(),
                    status: "open",
                    ...
                    subcontractors_detail:{
                        ...
                        client_change_rate:[
                            ...
                            // },
                            {
                                ...
                                subcontractors_id: subcon.id.toString(),
                                start_date: "2011-06-02 19:55:09",
                                end_date: null,
                ...
                all_given_creation_promises.push(helper.generate_timesheet(timesheet_may, "locked", 172.5, 8.00));
                        ...
                        try{
                            ...
                            InvoiceModel.findOne({}).exec(function(err, createdInvoice){
                                ...
                                try {
                                    ...
                                    expect(createdInvoice.pay_before_date ).to.equalDate(moment("2017-06-08T16:00.00Z").toDate());
                                    ...
                                    expect(createdInvoice.sub_total).to.equal(1562.06);
                                    expect(createdInvoice.gst_amount).to.equal(156.21);
                                    expect(createdInvoice.total_amount).to.equal(1718.27);
                                    ...
                                    //test second item - undertime
                                    var second_item = createdInvoice.items[1];
                                    expect(second_item.current_rate).to.equal(38);
                                    expect(second_item.description).to.equal("Efigenio IV Rodriguez [Full Time – Front End Developer] (May Un-Used Hour) Adjustment Credit Memo");
                                    ...
                                    expect(second_item.item_type).to.equal("Adjustment Credit Memo");
                                    expect(second_item.job_designation).to.equal("Full Time – Front End Developer");
                                    expect(second_item.qty).to.equal(-11.5);
                                    expect(second_item.unit_price).to.equal(9.37);
                                    expect(second_item.amount).to.equal(-107.75);
                                    ...
                                    expect(third_item.description).to.equal("Currency Adjustment (Contract Rate 1 AUD = 38 PESO VS Current Rate 1 AUD = 37.5 PESO, Currency Difference of 0.5  PESO for your staff Efigenio IV Rodriguez)(Actual Working Hours of 172.5 from May 1, 2017 to May 31, 2017)(Hourly Rate 9.37)/Current Rate 37.5");
                                    ...
                                    expect(third_item.qty).to.equal(172.5);
                                    expect(third_item.unit_price).to.equal(0.12);
                                    expect(third_item.amount).to.equal(20.7);
